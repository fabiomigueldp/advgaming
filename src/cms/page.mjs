...
// Base page fields (shared across all locales)
const getPageFields = () => [
    {
      label: t("title_seo"),
      name: "title",
...
// Generate page collection for each locale
const createPageCollection = (locale) => ({
  name: `page_${locale}`,
  identifier_field: "name",
  folder: `src/content/page/${locale}`,
  label: `Pages (${LOCALE_LABELS[locale]})`,
  format: "frontmatter",
  extension: "mdx",
  icon: "page",
  create: true,
...
export const page_en = createPageCollection('en');
export const page_pt = createPageCollection('pt');
export const page_es = createPageCollection('es');
export const page_fr = createPageCollection('fr');

// Export all page collections as an array
export const pageCollections = SUPPORTED_LOCALES.map(createPageCollection);

// Legacy export for backwards compatibility (defaults to English)
export const page = page_en;
