  ...
  <span class="contents">
    ...
    <transition name="nested">
      ...
      >
        ...
        <div
          class="inner mt-4 flex flex-wrap justify-center gap-2"
          :style="`--animation-delay: ${(menu.length + 1) * 100}ms`"
        >
          <a
            ...
            @click="toggleMenu()"
            class="border-current/20 rounded-lg border px-3 py-2 text-sm transition-all hover:bg-primary hover:text-white"
...
const getLocalizedPath = (path, targetLocale = null) => {
  const locale = targetLocale || props.currentLocale;
