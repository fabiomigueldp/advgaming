<template>
  <form @submit.prevent="submit" class="grid gap-4">
    ...
    <div class="input-group">
      <textarea
        class="hide-scrollbar peer max-h-32 resize-none"
        name="message"
        id=""
        placeholder=" "
        cols="30"
...
const formatPhone = (e) => {
  ...
  // Limit to 15 digits (ITU-T E.164 international standard)
  ...
  if (hasPlus && digits.length > 0) {
    ...
    if (digits.length <= 2) {
      ...
      form.phone = `(${digits.slice(0, 2)}) ${digits.slice(2, 7)}-${digits.slice(7)}`;
    }
  } else {
    form.phone = '';
  }
