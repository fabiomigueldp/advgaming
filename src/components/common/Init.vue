...
import { showPopup, showDialog, currentLocale } from "@src/store";
...
onMounted(() => {
  ...
  /* INITIALIZE CURRENT LOCALE FROM HTML LANG ATTRIBUTE */
  const htmlLang = html.getAttribute("lang");
  if (htmlLang && ["en", "pt", "es", "fr"].includes(htmlLang)) {
    currentLocale.set(htmlLang);
  }
  ...
  const handleAnchorClicks = () => {
    const anchorLinks = document.querySelectorAll(
      'a[href^="#"]:not([href="#contact"]):not([href="#sign-in"]):not([href="#sign-up"])',
    );
  ...
  // Handle anchor links after DOM is ready
  handleAnchorClicks();
  setTimeout(handleAnchorClicks, 500);
  ...
  const registerDialogListeners = () => {
    ...
    dialogs.forEach((el) => {
      ...
      buttons.forEach((btn) => {
        ...
        btn.removeEventListener("click", btn._dialogHandler);

        btn._dialogHandler = (e) => {
          e.preventDefault();
          showDialog.set({
            show: true,
            link: link,
        ...
        btn.addEventListener("click", btn._dialogHandler);
      });
    });
  };
  ...
  // Re-register after DOM changes (for dynamically loaded content)
  setTimeout(registerDialogListeners, 500);
  setTimeout(registerDialogListeners, 1000);
